...
## OSI의 애플리케이션 계층을 통한 애플리케이션 계층에 대한 이해
- TCP/IP 방식은 OSI 방식에 비해 간단하지만 애플리케이션 계층의 구현은 개발자의 몫
- OSI 방식을 통해 애플리케이션 계층을 세분화하여 봄으로써 애플리케이션 계층에 대한 이해를 높일 수 있음
...
---
## HTTP
- 애플리케이션 계층과 전송 계층 사이의 연결 관계로 동작하며 요청 및 응답 메세지로 대응됨
...
- 가령, ```www.naver.com```의 IP 주소가 222.111.222.111에서 222.111.222.122로 바뀌더라도 DNS를 통해 똑같은 ```www.naver.com```이라는 주소로 서비스 제공 가능
