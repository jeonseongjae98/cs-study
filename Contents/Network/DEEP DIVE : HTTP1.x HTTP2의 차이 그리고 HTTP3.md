...
이러한 문제를 해결하기 위해 HTTP/3는 QUIC이라는 프로토콜 위에서 동작한다. QUIC TCP의 신뢰성 보장을 위해 제공되는 기능들을 UDP 기반으로 직접 구현하여 성능을 개선한, 구글이 2013년에 공개한 프로토콜 이다. 따라서 HTTP/3는 HTTP over QUIC이라는 이름으로도 불린다.
